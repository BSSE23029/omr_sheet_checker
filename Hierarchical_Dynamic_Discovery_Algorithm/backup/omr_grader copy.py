...
def main():
    ...
    kernel = np.ones((3, 3), np.uint8)  # 3x3 square kernel for morphological operations
    ...
    for q in range(1, 49):  # Process all 48 questions
        ...
        for b in bubbles:
            ...
            mask = np.zeros(binarized.shape, dtype=np.uint8)  # Black background
            ...
            # - mask: 0 = outside bubble, 255 = inside bubble
    ...
    results.append({'question number': 'Total', 'correct answer': total, 'student answer': obtained, 'marks': ''})
    ...
    for q in range(1, 49):  # Process all 48 questions
        ...
        for idx, b in enumerate(bubbles):
            ...
            if idx == 0:
                ...
                # text_size is (width, height) tuple
                (text_width, text_height), _ = cv2.getTextSize(q_text, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)
